...
- [ ] Configure Auth.js (email + at least one social provider) — **partially done:** Google + Resend magic-link sign-in are wired end-to-end in code; `AUTH_GOOGLE_ID`/`AUTH_GOOGLE_SECRET`/`AUTH_RESEND_KEY` env vars exist on the deployed Vercel project (set before this project connected Vercel — see the 2026-09-19 session summary in CLAUDE.md) but `/signin` on the live site still reports "not configured," meaning those values aren't actually functioning — same pattern as `DATABASE_URL` and `AUTH_SECRET` turned out to be for the same batch. Needs checking directly in the Vercel dashboard
...
- [ ] Implement the daily reminder: Web Push primary, email (Resend) fallback, timed to the learner's usual practice window — **partially done:** Web Push subscription capture, a reminder-hour setting, and a manually-triggered send script (`npm run reminders:send`) are all built and tested (subscription flow verified up to the browser's own permission-grant prompt, which can't be automated); the Resend email fallback is scaffolded but inactive without real credentials (see the M0 Auth.js item — its Resend key is also unconfirmed working). Actually sending on a schedule needs a Vercel Cron job wired to an API route running the send script — Vercel itself is connected now (2026-09-19), but this specific cron job hasn't been built yet
...
- [ ] Soft launch to a small cohort — **partially unblocked:** the real deployment now exists (2026-09-19, see M0), but sign-in doesn't actually work there yet (Google/Resend credentials present but non-functional — see the M0 Auth.js item), and `/admin/*` still has zero access control (M1 follow-up) — inviting real people before either of those is resolved would mean nobody can save progress, and anyone could vandalize the lesson library. Resolve both before treating this as ready
