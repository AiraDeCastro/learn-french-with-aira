...
- **Don't assume the dev database is still running because it was earlier in the session.** It has stopped unexpectedly mid-session more than once (background processes here don't reliably persist across tool calls). If a commit fails with `ECONNREFUSED`, run `npx prisma dev ls` to check, then `npx prisma dev start aira-dev` if needed, and just retry the commit — nothing is lost when the gate fails, git aborts the commit cleanly.
...
M0 (Project Foundations) is done except account-provisioning (hosted Postgres, Cloudflare R2, Vercel, Sentry, PostHog) that only Aira can do. M1 (Content Model & Admin Panel) and M2 (Reader/Player Core) are both mostly done — see [docs/TASKS.md](docs/TASKS.md) for the exact checklist. The learner-facing reader now works end-to-end at `/learn/[id]`: synced text, tap-to-translate with real word-saving, a comprehension check that records completions, and offline-tolerant queueing — all tested live in-browser against the real local database, not just code-reviewed.
