...
M0 (Project Foundations) is done except account-provisioning (hosted Postgres, Cloudflare R2, Vercel, Sentry, PostHog) that only Aira can do. M1 (Content Model & Admin Panel) is mostly done — see [docs/TASKS.md](docs/TASKS.md) for the exact checklist. Everything code-side is in place: Next.js + TypeScript + Tailwind, ESLint/Prettier, tRPC, Prisma (working against a local dev database), Auth.js (Google + email sign-in wired, needs real credentials to activate), GitHub Actions CI, Vitest/Playwright test runners, a Prisma schema covering the full domain model, and a working admin panel for creating/editing lessons. Phase 2/3 additions (CMS, speech-to-text, etc.) aren't decided yet — flag those to the user when that work comes up.

Two things in M1 are genuinely blocked, not just deferred: **real audio narration** (needs an actual human voice — 4 mini-stories are written and seeded, but `audioUrl` stays null until someone records them) and **admin access control** (`/admin/*` and `/api/admin/upload` have zero auth right now — fine for solo local work, not fine to deploy anywhere public).
